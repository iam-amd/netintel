...
export function recommendation(c: UiCustomer): { title: string; detail: string } {
  if (c.fiberSignal < -30) {
    return {
      title: "Dispatch field tech",
      detail: "Signal is degraded — schedule a line audit.",
    };
  }
  if (c.latePayments >= 3) {
    return {
      title: "Offer payment plan",
      detail: "Multiple late payments suggest payment friction.",
    };
  }
  if (c.recentOutages >= 4) {
    return {
      title: "Network ticket",
      detail: "Service reliability is below target in this area.",
    };
  }
  if (c.ticketsRaised >= 5) {
    return {
      title: "Care call-back",
      detail: "High support load — assign a senior agent.",
    };
  }
  if (!c.autopay) {
    return {
      title: "Enroll in autopay",
      detail: "Reduces involuntary churn by ~15%.",
    };
  }
  return {
    title: "Continue monitoring",
    detail: "No urgent intervention required.",
  };
}
